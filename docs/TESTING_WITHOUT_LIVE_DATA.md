...
## Testing against a richer scenario: `--scenario`

The default 4-station data above is one fixed scenario. For more
variety — different station counts, deliberate failure modes,
realistic noise, multiple superimposed waves — `mock_psws_server.py`
can also serve any of `synthetic_tests/`'s real test conditions.

**GUI option:** `mock_server_gui.py` walks through picking a scenario,
starting/stopping the server, and downloading the result — no second
terminal, and no manual `export PSWS_BASE_URL=...` (it's set directly
in the download subprocess's own environment, scoped to exactly the
port the managed server is listening on):

```bash
python3 mock_server_gui.py
```

The command-line steps below do exactly the same thing under the
hood, useful if you want to see or script each step directly.
...
**The 6 recommended, and why each earns its place** (out of 30 total —
most of the rest are speed/azimuth/period/SNR sweeps around these same
core ideas, valuable for the real automated test suite's statistical
coverage but redundant for a hands-on demo):

| Scenario | Ground truth | Array | Demonstrates |
|---|---|---|---|
| `nominal` | 500 m/s @ 30° | 3 stations | Clean baseline — the reference success case |
| `slow_tid_alias` | 150 m/s @ 30° | 3 stations | **Failure mode 1**: aliasing — lag exceeds period/2, wrong peak locked |
| `very_low_snr` | 500 m/s @ 30° | 3 stations | **Failure mode 2**: weak signal — a genuinely different kind of failure than aliasing |
| `mixed_4stn` | 509 m/s @ 137° | 4 stations | Confirms the pipeline isn't hardcoded to always-3-stations |
| `conus_5stn` | 682 m/s @ 63° | 5 stations | Ground truth deliberately matches this project's own real 19 January 2026 reference event exactly — compare the clean synthetic result directly against that real, noisier one |
| `two_wave` | 500 m/s @ 30° (+ secondary 200 m/s @ 270°) | 3 stations | Two genuinely superimposed TIDs — the richest part of the signal model |

Usage is otherwise identical to the default scenario — same download
and workflow commands, just with each scenario's own station names
(shown in the server's startup banner) and its own test date (also
shown at startup; **not** the same as this project's real reference
event date, confirmed directly rather than assumed, so there's no risk
of confusing the two or overwriting real downloaded data).

**Generation can take a while the first time** — anywhere from a few
seconds to over a minute for scenarios with more stations or a longer
duration (results are cached under `synthetic_tests/events/` afterward,
same convention the real `synthetic_tests/run_tests.py` suite already
uses, so a scenario generated by either one is reused by the other).
**Let it finish rather than interrupting it** — killing generation
partway through (an impatient Ctrl+C, or connecting before the startup
banner prints) leaves a broken, partially-written cache that fails
every future attempt to regenerate that same scenario, until the
stale `synthetic_tests/events/synthetic_<name>/` directory is deleted
by hand.

Each scenario's own known-correct answer lives in
`synthetic_tests/events/synthetic_<name>/ground_truth.json` once
generated — including its stated pass/fail criteria, useful for
judging whether a recovered result is actually within the expected
range rather than just "in the right ballpark."
