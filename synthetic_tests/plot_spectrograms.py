...
def make_spectrogram(iq, fs_hz, nfft=NFFT, hop_s=HOP_S):
    ...
    for i in range(n_frames):
        ...
        S = np.fft.fftshift(np.fft.fft(frame))
        spec[:, i] = np.abs(S) ** 2
...
def plot_station(station_name, station_dir, event_start_unix, duration_s,
    ...
    if output_path:
        ...
        # Write sidecar axes.json for tid_spect_click.py pixel mapping
        import json as _json
        renderer = fig.canvas.get_renderer()
        bbox = ax.get_window_extent(renderer=renderer)
        fig_w, fig_h = fig.get_size_inches() * fig.dpi
        pf = [bbox.x0/fig_w, bbox.x1/fig_w, bbox.y0/fig_h, bbox.y1/fig_h]
        ...
        sidecar = {
            ...
            't_end_utc_hours':   t1_h,
            'doppler_lo_hz':    -SEARCH_HZ,
            'doppler_hi_hz':     SEARCH_HZ,
            'dpi': 120,
            'plot_fraction': pf,
            'date_utc': date_utc,
            '_note': 'Generated by plot_spectrograms.py for use with tid_spect_click.py'
        }
        sidecar_path = output_path.parent / (output_path.stem + '_axes.json')
