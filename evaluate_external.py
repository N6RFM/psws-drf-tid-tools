#!/usr/bin/env python3
"""
validate_external.py — external space weather validation for TID DOA results
...
def write_report(out_dir, args, ev_start, ev_end,
    ...
    lines = [
        ...
        f"psws-drf-tid-tools validate_external.py v{VERSION}",
        hr('═'),
    ...
    lines += [
        ...
        "",
        hr('═'),
        f"Generated by validate_external.py v{VERSION}",
        f"psws-drf-tid-tools — https://github.com/N6RFM/psws-drf-tid-tools",
        hr('═'),
    ]

    rpt = pathlib.Path(out_dir) / "validation_report.txt"
    rpt.write_text("\n".join(lines) + "\n", encoding='utf-8')
    print(f"    → {rpt.name}")
...
def main():
    ...
    p = argparse.ArgumentParser(
        description="External space weather validation for TID DOA results",
    ...
    print(f"\n{'='*55}")
    print(f"validate_external.py v{VERSION}")
    ...
    print("  Run find_event_stations.py for cross-validation DOA")
