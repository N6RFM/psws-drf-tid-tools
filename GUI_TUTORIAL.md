...
## Advanced: sgolay-ridge extraction (recommended for contaminated stations)

After clicking corridor and pressing X, instead of writing a sinusoid-fitted
CSV, re-run drf_to_doppler.py with the corridor and sgolay-ridge method.
This uses the full 2D spectrogram and is more physically correct.

**Step 1: Generate corridor JSON** (as in main tutorial, press X)

**Step 2: Re-run extraction with sgolay-ridge**

    python3 drf_to_doppler.py ./w7lux \
        --subchannel 0 \
        --start 2024-05-17T18:00:00 \
        --end 2024-05-17T20:00:00 \
        --decim-seconds 60 \
        --method sgolay-ridge \
        --corridor w7lux_fft_clean_corridor.json \
        --sgolay-window 21 \
        --output w7lux_sgolay_ridge.csv

**Step 3: Use sgolay_ridge.csv in DOA config instead of automated CSV**

Key parameters:
- --sgolay-window: smoothing window in minutes (default 21)
  - 21 min: good for LSTID (60 min period) — removes spikes, preserves phase
  - 10 min: good for MSTID (15-30 min period)
- --corridor: required, must be generated by tid_spect_click.py (X key)

Best results when applied to ALL stations with consistent corridors.

---

## Preview mode (--drf-dir)

Pass --drf-dir to enable automatic sgolay-ridge preview after X:

    python3 tid_spect_click.py \
        --spectrogram w7lux_tid_zoom.png \
        --csv w7lux_fft_tid2.csv \
        --name W7LUX \
        --drf-dir ./w7lux \
        --subchannel 0 \
        --sgolay-window 21
...
After pressing X:
1. Corridor JSON is written
2. Consistency check runs
3. sgolay-ridge extraction runs automatically (~10-30s)
4. Result shown as GREEN curve on spectrogram

Interpreting the green curve:
- Green tracks carrier within corridor bounds ✅ — corridor is good
- Green goes out of bounds at edges — corridor needs more clicks there
- Green follows wrong feature — re-click corridor (press R)
- No green — extraction failed, check terminal for error

Press R to reset and re-click if preview looks wrong.
Press Q to accept and quit if preview looks correct.
